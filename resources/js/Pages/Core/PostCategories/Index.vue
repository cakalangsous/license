...
import post_categories from "@/routes/admin/post_categories";
...
const submit = () => {
    if (isEditing.value) {
        form.put(
            post_categories.update.url({ post_category: editingId.value }),
            {
                onSuccess: () => {
        ...
        form.post(post_categories.store.url(), {
            onSuccess: () => {
...
const deleteItem = () => {
    if (itemToDelete.value) {
        router.delete(
            post_categories.destroy.url({
                post_category: itemToDelete.value.id,
            }),
            {
                onSuccess: () => {
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div class="lg:col-span-8">
                <Card :title="`${title} List`">
                    <DataTable
                        :paginated-data="paginatedCategories"
                        :columns="columns"
                    >
